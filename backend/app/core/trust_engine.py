...
Design Principle:
    RAW DATA -> VALIDATION -> CROSS-SENSOR CONSISTENCY -> PHYSICAL PLAUSIBILITY
    -> TEMPORAL CONSISTENCY -> DATA TRUST SCORE -> DIGITAL TWIN
"""
...
class TrustEngine:
    ...
    def check_temporal_consistency(self, device_id: str, new_value: float, max_jump_per_sec: float = 15.0) -> Tuple[bool, str]:
        """
        ...
        dt = max(0.001, time.time() - last_ts)
    ...
    def evaluate_sensor_consensus(
        ...
        # Multiple sources: calculate weighted mean and variance
        total_weight = 0.0
        weighted_sum = 0.0
        values = []
        source_evals = []
        ...
        # Check deviations from consensus
        conflicts = []
        ...
        for obs in observations:
            ...
            # If deviation is > 15 units or > 25% from independent majority
            is_outlier = False
            # Check against median or independent sources
            other_vals = [o.value for o in observations if o.device_id != obs.device_id]
            median_others = sorted(other_vals)[len(other_vals)//2]
            if abs(obs.value - median_others) > 20.0:
                is_outlier = True

            current_trust = base_trust
            if is_outlier:
                # Degrade trust score for this corrupted or anomalous device
        ...
        # Calculate final trusted state estimation excluding heavily degraded outliers
        if valid_weight > 0:
            final_trusted_value = valid_weighted_sum / valid_weight
        else:
            final_trusted_value = raw_weighted_mean
